...
nmap -sV -sC -p $PORT --script="banner,vuln,(ssh*) and not (brute or broadcast or dos or external or fuzzer)" -oN "$REPORT/$SERVICE_U-nmap-$SERVICE_L-$PORT.txt" $IP > /dev/null 2>&1 &
pids[${RANDOM}]=$!
